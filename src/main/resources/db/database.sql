...
-- =============================================
-- 使用说明:
-- 1. 新建数据库时直接执行此脚本即可创建所有表
-- 2. 此脚本为全量脚本，包含所有版本的最终表结构
-- =============================================
-- 版本历史:
-- v1.0.0 - 初始版本，创建 12 张基础表
-- v1.1.0 - 修正 users.nickname VARCHAR(100) → VARCHAR(50)，与实体 length=50 一致
--           保留 announcement_reads 唯一约束 uk_announcement_user
-- v1.2.0 - 表结构无变更，业务逻辑层面改进：
--           1. joinRoom 新增黑名单检查（使用现有 room_blacklist 表）
--           2. updateRoom 新增支持修改 type 和 password 字段（使用现有 chat_rooms 表字段）
--           3. 新增 transferOwnership 转让聊天室所有权（操作 room_members 和 chat_rooms 表）
--           4. WebSocket 消息发送失败时通过 /user/queue/errors 反馈错误
--           5. AdminService 仪表盘统计 bannedUsers 改用聚合查询
-- =============================================
...
--   1. users              - 用户表
--   2. chat_rooms         - 聊天室表
--   3. room_members       - 聊天室成员关联表
--   4. messages           - 消息表
--   5. room_blacklist     - 聊天室黑名单表
--   6. announcements      - 公告表
--   7. announcement_reads - 公告阅读记录表
--   8. banned_users       - 禁用用户表
--   9. reports            - 举报表
--  10. system_logs        - 系统日志表
--  11. email_verifications - 邮箱验证表
--  12. email_send_logs    - 邮件发送日志表
-- =============================================
